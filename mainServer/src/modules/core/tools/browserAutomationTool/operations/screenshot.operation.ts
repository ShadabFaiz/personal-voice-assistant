...
export class ScreenshotOperation implements IBrowserOperation {
  ...
  async execute(params: BrowserOperationParams): Promise<string> {
    ...
    const filename = payload?.filename || `/screenshots/screenshot_${Date.now()}.png`;
    const buffer = await page.screenshot({ fullPage: payload?.fullPage || false });
    ...
    if (response[0] !== null) {
      throw new Error(`Failed to save screenshot via FileTool: ${response[0].message}`);
    }
